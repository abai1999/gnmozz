"""Direct local alignment controller v3.

Near-contact controller that predicts a bounded local residual directly rather
than ranking a fixed proposal set.  The controller is intentionally scoped to
near/micro alignment and is meant to run in shadow first.
"""
...
class AlignmentV3DirectLocalController(nn.Module):
    """Target-conditioned near-contact direct local controller.

    Inputs:
    - `wrist_depth`: `(B, 1, H, W)` depth crop around the end-effector
    - `force_history`: `(B, T, 6)`
    - `proprio`: `(B, P)`
    - `planner_action_local`: `(B, 6)` planner coarse action in EE-local frame
    - `current_to_target_delta_local`: `(B, 6)` signed target-relative delta

    Outputs:
    - `direct_residual_4d`: `(B, 4)` -> `[dx, dy, dz, dyaw]`
    - `direct_residual_6d`: `(B, 6)` with roll/pitch zero-filled for runtime
    - `shadow_post_xyz_yaw`: predicted post-error diagnostics for shadow audit
    - `risk_logit`: invalid/overshoot proxy for shadow gating
    - `confidence_logit`: scalar confidence for future apply decisions
    """
    ...
    def __init__(
        ...
        planner_dim: int = 32,
        target_dim: int = 64,
        max_pos: float = 0.0125,
        max_yaw: float = 0.0020,
        use_planner_action: bool = True,
        use_wrist_depth: bool = True,
        use_force: bool = True,
    ) -> None:
        super().__init__()
        self._controller_type = "alignment_v3_direct_local_controller"
        ...
        fused_dim = depth_dim + force_dim + proprio_dim + planner_dim + target_dim
        ...
        self.post_error_head = nn.Sequential(
            nn.Linear(state_dim, hidden_dim),
            nn.ReLU(inplace=True),
        ...
        # Start from an almost-noop controller; let training move it away.
        for module in (self.residual_head[-1], self.post_error_head[-1], self.risk_head[-1], self.confidence_head[-1]):
    ...
    def encode_state(
        ...
        bsz = planner_action_local.shape[0]
        dtype = planner_action_local.dtype
        device = planner_action_local.device
        ...
        if self.use_wrist_depth:
            ...
            parts.append(torch.zeros((bsz, 64), device=device, dtype=dtype))
        if self.use_force:
        ...
        if self.use_planner_action:
            ...
            parts.append(torch.zeros((bsz, 32), device=device, dtype=dtype))
    ...
    def forward(
        ...
        post_error_xyz_yaw = self.post_error_head(state)
