...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def _regression_or_discrete_prediction(
        ...
        for item in language_model_output.hidden_states[0:]:
            ...
            # Action hidden states: skip BOS (+1) + patches + prompt tokens
            actions_hidden_states = text_hidden_states[:, NUM_PATCHES + 1 + NUM_PROMPT_TOKENS : NUM_PATCHES + 1 + NUM_PROMPT_TOKENS + NUM_TOKENS, :,].reshape(1, 1, NUM_TOKENS, -1).to(torch.bfloat16)
            
            batch_size = item.shape[0]
            # Task hidden states: skip BOS, take only patch embeddings
        ...
        if action_head is not None:
            ...
            predicted_action_token_ids = (
                language_model_output.logits[
                    :,
                    NUM_PATCHES + 1 + NUM_PROMPT_TOKENS : NUM_PATCHES + 1 + NUM_PROMPT_TOKENS + ACTION_DIM * NUM_ACTIONS_CHUNK,
                ]
                .argmax(dim=2)
                .cpu()
                .numpy()
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        if depth is not None:
            depth = torch.as_tensor(depth, device=projected_patch_embeddings.device, dtype=projected_patch_embeddings.dtype)

        if force_history is not None:
            force_history = torch.as_tensor(force_history, device=projected_patch_embeddings.device, dtype=projected_patch_embeddings.dtype)
