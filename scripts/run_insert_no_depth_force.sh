#!/bin/bash
# ─────────────────────────────────────────────────────────────────────
# Ablation: insert_onto_square_peg WITHOUT depth/force (vision-only)
# Same hyperparams as run_insert_long_train.sh for fair comparison, but
# machine-specific settings are overridable via env vars.
# ─────────────────────────────────────────────────────────────────────
set -euo pipefail
...
RUN_ID_NOTE="${RUN_ID_NOTE:-insert_vision_only_50k_$(date +%Y%m%d_%H%M%S)}"
...
echo "=== Starting insert_onto_square_peg (NO depth/force) training (50k steps) ==="
