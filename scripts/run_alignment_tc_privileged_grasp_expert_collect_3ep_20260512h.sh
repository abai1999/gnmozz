...
TAG="${TAG:-20260512h_grasp_expert_3ep}"
...
GRASP_BASIN_PROFILE_JSON="${GRASP_BASIN_PROFILE_JSON:-$ROOT/runtime_artifacts/alignment_tc_diffusion/grasp_basin_audit_20260513a/grasp_basin_profile_20260513a.json}"
...
env xvfb-run -a "$PYTHON_BIN" "$ROOT/scripts/evaluate_rlbench.py" \
  ...
  --grasp_basin_profile_json "$GRASP_BASIN_PROFILE_JSON" \
  ...
  --alignment_tc_teacher_broad_max_yaw_step 0.025 \
  --alignment_tc_teacher_near_max_pos_step 0.0025 \
  --alignment_tc_teacher_near_max_yaw_step 0.020 \
  --alignment_tc_teacher_micro_max_pos_step 0.0012 \
  --alignment_tc_teacher_micro_max_yaw_step 0.015 \
