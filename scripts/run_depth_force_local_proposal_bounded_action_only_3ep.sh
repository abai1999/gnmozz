...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/runtime_artifacts/depth_force_contact/bounded_action_only_3ep_20260504a}"
...
STUDENT_CANDIDATE_EVAL_SHADOW_CKPT="${STUDENT_CANDIDATE_EVAL_SHADOW_CKPT:-$ROOT/runtime_artifacts/stage_refiner/b2_yawmode_dataset_v13_selected_profile_20260425c/offline_train_v14b_pilotmerge_ablation/student_candidate_evaluator_v2_best.pt}"
STUDENT_CANDIDATE_EVAL_HANDOFF_CKPT="${STUDENT_CANDIDATE_EVAL_HANDOFF_CKPT:-$ROOT/runtime_artifacts/stage_refiner/student_handoff_state_v2_phaseA_20260422l/train_run12_stage1b_calibmix/student_handoff_state_head_v2_best_phaseA_deploy.pt}"
...
exec env xvfb-run -a "$PYTHON_BIN" scripts/evaluate_rlbench_modes.py \
  ...
  --name_suffix "depth_force_local_proposal_final_full_bounded_action_only_3ep" \
  --alignment_ckpt "$ALIGNMENT_CKPT" \
  --student_handoff_shadow_only \
  --student_candidate_evaluator_shadow_ckpt "$STUDENT_CANDIDATE_EVAL_SHADOW_CKPT" \
  --student_candidate_evaluator_handoff_ckpt "$STUDENT_CANDIDATE_EVAL_HANDOFF_CKPT" \
