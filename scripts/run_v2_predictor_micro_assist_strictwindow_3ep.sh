...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/runtime_artifacts/depth_force_contact/v2_predictor_micro_assist_strictwindow_3ep_20260506a}"
...
exec env xvfb-run -a "$PYTHON_BIN" scripts/evaluate_rlbench_modes.py \
  ...
  --name_suffix "v2_predictor_micro_assist_strictwindow_3ep" \
  --alignment_ckpt "$ALIGNMENT_CKPT" \
  --v2_alignment_shadow_ckpt "$V2_CKPT" \
  --v2_target_delta_provider_ckpt "$TD_CKPT" \
  ...
  --alignment_near_zone_xy_threshold 0.03 --alignment_near_zone_z_threshold 0.07 \
  --enable_v2_predictor_micro_assist_apply \
  --v2_predictor_micro_assist_scale_cap 0.10 \
  --v2_predictor_micro_assist_max_pos 0.00125 \
  --v2_predictor_micro_assist_max_rot 0.0060 \
