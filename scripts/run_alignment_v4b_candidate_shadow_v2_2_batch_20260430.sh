...
EPISODE_INDICES="${EPISODE_INDICES:-8,10,16,17,19,20}"
BATCH_LABEL="${BATCH_LABEL:-v2_2_yawneeded}"
...
/home/guoning/my_conda_envs/vla-adapter/bin/python "$ROOT/scripts/audit_alignment_v4b_yaw_needed_failures.py" \
  --trace_dir "$TRACE_DIR" \
  --output_json "$OUTPUT_ROOT/alignment_v4b_yaw_needed_failure_audit.json" \
  --keep_yaw_abs "$KEEP_YAW_ABS"
