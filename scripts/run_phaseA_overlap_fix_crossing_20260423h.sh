...
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT/runtime_artifacts/stage_refiner/student_handoff_state_v2_phaseA_overlapfix_crossing_20260423h/train_overlapfix_crossing_small}"

# Keep v1 best_ready as the most reliable overlap-fix starting point so far.
...
python "$ROOT/scripts/train_student_handoff_state_head_v2.py" \
  ...
  --lambda_close_ready_crossing 0.45 \
  --crossing_xy_max 1.05 \
  --crossing_z_max 1.00 \
  --crossing_yaw_max 1.00 \
  --ready_pos_margin_logit 1.15 \
...
echo "[phaseA-overlap-fix-crossing] output_dir=$OUTPUT_DIR"
