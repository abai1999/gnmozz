#!/usr/bin/env python3
"""Train alignment_v4_direct_local_controller."""
...
from prismatic.vla.datasets.alignment_v4_direct_local_dataset import AlignmentV4DirectLocalDataset
...
def _bucket_shadow_metrics(batch: dict, out: dict, noop_pos_epsilon: float, noop_yaw_epsilon: float) -> dict[str, dict[str, float]]:
...
def _compute_loss(batch: dict, out: dict, args) -> tuple[torch.Tensor, dict[str, float]]:
    cur = batch["current_to_target_delta_local"].float()
    ...
    pred_reduction = out["shadow_delta_reduction"].float()
    target_reduction = torch.stack(
        [
            torch.clamp(cur[:, 0].abs() - target_post[:, 0], min=-1.0, max=1.0),
            torch.clamp(cur[:, 2].abs() - target_post[:, 1], min=-1.0, max=1.0),
            torch.clamp(cur[:, 5].abs() - target_post[:, 2], min=-1.0, max=1.0),
        ],
        dim=-1,
    )
    invalid = batch["invalid_risk_proxy"].float()
    overshoot = batch["overshoot_proxy"].float()
    risk_target = torch.maximum(invalid, overshoot)
    conf_target = (
        batch["target_improves_xy"].float()
        * batch["target_improves_z"].float()
        * batch["target_improves_yaw"].float()
    )
    policy_target = batch["policy_mode_label"].long()
    sample_weight = batch["sample_weight"].float()

    l_residual = F.smooth_l1_loss(pred_residual[:, :3], target_residual[:, :3], reduction="none").mean(dim=-1)
    l_residual_yaw = F.smooth_l1_loss(pred_residual[:, 3], target_residual[:, 3], reduction="none")
    l_post_xy = F.smooth_l1_loss(pred_post[:, 0], target_post[:, 0], reduction="none")
    l_post_z = F.smooth_l1_loss(pred_post[:, 1], target_post[:, 1], reduction="none")
    l_post_yaw = F.smooth_l1_loss(pred_post[:, 2], target_post[:, 2], reduction="none")
    l_reduction = F.smooth_l1_loss(pred_reduction, target_reduction, reduction="none").mean(dim=-1)
    l_risk = F.binary_cross_entropy_with_logits(out["risk_logit"].float(), risk_target, reduction="none")
    l_conf = F.binary_cross_entropy_with_logits(out["confidence_logit"].float(), conf_target, reduction="none")
    l_mode = F.cross_entropy(out["policy_mode_logits"].float(), policy_target, reduction="none")
    l_norm = pred_residual.pow(2).mean(dim=-1)

    total = (
        1.0 * l_post_xy
        + 1.2 * l_post_z
        + float(args.yaw_weight) * l_post_yaw
        + 0.8 * l_residual
        + float(args.yaw_weight) * l_residual_yaw
        + 0.5 * l_reduction
        + 0.4 * l_risk
        + 0.25 * l_conf
        + 0.25 * l_mode
    ...
    total = (total * sample_weight).mean()
    stats = {
        "loss": float(total.item()),
        "l_post_xy": float((l_post_xy * sample_weight).mean().item()),
        "l_post_z": float((l_post_z * sample_weight).mean().item()),
        "l_post_yaw": float((l_post_yaw * sample_weight).mean().item()),
        "l_residual": float((l_residual * sample_weight).mean().item()),
        "l_residual_yaw": float((l_residual_yaw * sample_weight).mean().item()),
        "l_reduction": float((l_reduction * sample_weight).mean().item()),
        "l_risk": float((l_risk * sample_weight).mean().item()),
        "l_conf": float((l_conf * sample_weight).mean().item()),
        "l_mode": float((l_mode * sample_weight).mean().item()),
        "l_norm": float((l_norm * sample_weight).mean().item()),
    }
    return total, stats
...
def _run_epoch(
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(train):
            ...
            loss, loss_stats = _compute_loss(batch, out, argparse.Namespace(yaw_weight=yaw_weight))
            if train:
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        ...
        shadow_rows[-1]["per_bucket"] = _bucket_shadow_metrics(batch, out, noop_pos_epsilon, noop_yaw_epsilon)
    ...
    summary = {
        ...
        "l_reduction": _mean("l_reduction", rows),
        "l_risk": _mean("l_risk", rows),
        "l_conf": _mean("l_conf", rows),
        "l_mode": _mean("l_mode", rows),
    ...
    for bucket in ("near_alignment", "micro_contact_refine"):
        prefix = f"{bucket}_"
        for key in ("xy_improved_rate", "z_improved_rate", "yaw_improved_rate", "all_improved_rate", "pred_pos_norm_mean", "pred_yaw_abs_mean", "near_noop_rate"):
...
def main() -> None:
    ...
    parser.add_argument("--yaw_weight", type=float, default=1.5)
    parser.add_argument("--max_pos", type=float, default=0.015)
    parser.add_argument("--max_yaw", type=float, default=0.006)
    ...
    parser.add_argument("--enable_front_rgb", action="store_true", default=False)
    ...
    full_ds = AlignmentV4DirectLocalDataset(str(args.dataset_npz), stage_bucket_filter=["near_alignment", "micro_contact_refine"])
    ...
    model = AlignmentV4DirectLocalController(
        ...
        use_front_rgb=args.enable_front_rgb,
    ...
    for epoch in range(args.epochs):
        ...
        composite = val_stats["shadow_all_improved_rate"] + 0.25 * val_stats["shadow_yaw_improved_rate"] + 0.1 * val_stats["shadow_xy_improved_rate"]
        print(
            f"epoch {epoch+1:03d} "
            ...
            torch.save(
                {
                    ...
                    "max_pos": args.max_pos,
                    "max_yaw": args.max_yaw,
                    "use_front_rgb": bool(args.enable_front_rgb),
                    "use_planner_action": use_planner_action,
                    "use_force": use_force,
                },
                output_dir / "alignment_v4_direct_local_best.pt",
            )
    ...
    torch.save(
        ...
        output_dir / "alignment_v4_direct_local_final.pt",
    )
    (output_dir / "alignment_v4_train_history.json").write_text(
        json.dumps(
            {
