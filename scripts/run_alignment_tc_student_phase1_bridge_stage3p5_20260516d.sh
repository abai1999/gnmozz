...
OUT_DIR="${OUT_DIR:-runtime_artifacts/alignment_tc_diffusion/train_alignment_tc_student_phase1_bridge_stage3p5_20260516d}"
...
env CUDA_VISIBLE_DEVICES="$CUDA_VISIBLE_DEVICES" "$PYTHON_BIN" scripts/train_alignment_tc_student_vnext.py \
  ...
  --max_pos_step 0.0015 \
  --max_yaw_step 0.0060 \
  --phase1_target_axis_weights 1.0,1.0,1.0,0.25,0.25,1.0 \
  --phase2_target_axis_weights 1.0,1.50,1.0,0.25,0.25,1.75 \
  --phase1_action_axis_weights 1.0,1.0,1.0,1.0 \
  --phase2_action_axis_weights 1.0,1.50,1.0,1.75 \
  --phase1_yaw_dir_weight 0.75 \
  --phase2_yaw_dir_weight 1.00 \
  --phase1_verified_weight_boost 1.00 \
  --enable_phase1_bridge_repair_losses \
  --phase1_sign_y_weight 0.10 \
  --phase1_sign_yaw_weight 0.0 \
  --phase1_mag_floor_y_weight 0.04 \
  --phase1_mag_floor_yaw_weight 0.0 \
  --enable_decoupled_y_bridge \
  --y_bridge_max_step 0.0008 \
  --freeze_main_residual_for_y_bridge
