...
OUT_ROOT="${OUT_ROOT:-$ROOT/runtime_artifacts/stage_refiner/phaseA_alignment_v3_teacher_augmented_20260427}"
...
DEFAULT_TEACHER_SUPPORT_NPZ="$ROOT/runtime_artifacts/residual_data/insert_phase1_teacher_support_states_demo_grasp_yaw_20260419a_parallel_20260428c/support_states.npz"
...
TEACHER_DIR="$DATASET_DIR/teacher_augmented"
...
mkdir -p "$DATASET_DIR" "$STAGEA_DIR" "$STAGEB_DIR" "$TEACHER_DIR"

echo "[alignment-v3] OUT_ROOT=$OUT_ROOT"
echo "[alignment-v3] RECOLLECT_SUPPORT_NPZ=$RECOLLECT_SUPPORT_NPZ"
echo "[alignment-v3] AUGMENT_SUPPORT_NPZ=${AUGMENT_SUPPORT_NPZ:-}"
echo "[alignment-v3] TEACHER_SUPPORT_NPZ=${TEACHER_SUPPORT_NPZ:-}"
...
teacher_support_args=()
canonical_teacher_support_npz="${TEACHER_SUPPORT_NPZ:-$DEFAULT_TEACHER_SUPPORT_NPZ}"
if [[ -f "$canonical_teacher_support_npz" ]]; then
  echo "[alignment-v3] using canonical teacher support npz: $canonical_teacher_support_npz"
  teacher_support_args+=(--support_npz "$canonical_teacher_support_npz" --source_name "${TEACHER_SOURCE_NAME:-phase1_yaw_teacher_20260419a_parallel}" --source_weight_mult "${TEACHER_SOURCE_WEIGHT_MULT:-1.0}")
elif [[ -f "${TEACHER_SUPPORT_NPZ:-$DEFAULT_TEACHER_SUPPORT_NPZ}" && "${ALLOW_VISUAL_POSEFIELD_FALLBACK:-0}" == "1" ]]; then
  teacher_support_npz="${TEACHER_SUPPORT_NPZ:-$DEFAULT_TEACHER_SUPPORT_NPZ}"
  echo "[alignment-v3] falling back to visual posefield teacher support npz: $teacher_support_npz"
  teacher_support_source_name="${TEACHER_SOURCE_NAME:-teacher_visual_posefield_merged_20260416a}"
  teacher_support_weight="${TEACHER_SOURCE_WEIGHT_MULT:-1.0}"
  teacher_support_args=(--support_npz "$teacher_support_npz" --source_name "$teacher_support_source_name" --source_weight_mult "$teacher_support_weight")
else
  echo "[alignment-v3] ERROR: no canonical teacher support npz found. Expected:" >&2
  echo "  - $canonical_teacher_support_npz" >&2
  echo "[alignment-v3] Re-run scripts/run_phase1_yaw_teacher_pipeline_20260419a_parallel_collect.sh to regenerate the canonical teacher lineage." >&2
  exit 1
fi

if [[ -n "${AUGMENT_SUPPORT_NPZ:-}" ]]; then
  IFS=':' read -r -a EXTRA_AUG_NPZS <<< "$AUGMENT_SUPPORT_NPZ"
  for extra_npz in "${EXTRA_AUG_NPZS[@]}"; do
    [[ -z "$extra_npz" ]] && continue
    [[ -f "$extra_npz" ]] || { echo "ERROR: augment support npz missing: $extra_npz" >&2; exit 1; }
    input_args+=(--input_npz "$extra_npz")
  done
fi

if [[ ${#teacher_support_args[@]} -gt 0 ]]; then
  TEACHER_ANCHOR_OUT="$TEACHER_DIR/teacher_success_anchor.npz"
  TEACHER_ANCHOR_META="$TEACHER_DIR/teacher_success_anchor.meta.json"
  TEACHER_ANCHOR_WINDOW_OUT="$TEACHER_DIR/teacher_success_anchor_window.npz"
  TEACHER_ANCHOR_WINDOW_META="$TEACHER_DIR/teacher_success_anchor_window.meta.json"
  "$PYTHON_BIN" "$ROOT/scripts/build_teacher_success_anchor_dataset.py" \
    "${teacher_support_args[@]}" \
    --output_npz "$TEACHER_ANCHOR_OUT" \
    --meta_json "$TEACHER_ANCHOR_META"

  "$PYTHON_BIN" "$ROOT/scripts/build_teacher_success_anchor_window_dataset.py" \
    --input_npz "$TEACHER_ANCHOR_OUT" \
    --output_npz "$TEACHER_ANCHOR_WINDOW_OUT" \
    --meta_json "$TEACHER_ANCHOR_WINDOW_META"

  input_args+=(--input_npz "$TEACHER_ANCHOR_WINDOW_OUT")
fi
...
"$PYTHON_BIN" "$ROOT/scripts/build_phaseA_alignment_v3_closeability_dataset.py" \
  ...
  --corrective_yaw_symmetry_period "${CORRECTIVE_YAW_SYMMETRY_PERIOD:-1.5707963267948966}" \
  --allowed_phase_id "${ALLOWED_PHASE_IDS:-1}" \
  --allowed_phase_id "${ALLOWED_PHASE_IDS_SECONDARY:-2}" \
  --allowed_stage_bucket "${ALLOWED_STAGE_BUCKETS:-grasp_align,grasp_closeable,transport_align,insert_align,insert_contact_pre}"
...
"$PYTHON_BIN" "$ROOT/scripts/audit_teacher_student_distill_chain.py" \
  --support_npz "$RECOLLECT_SUPPORT_NPZ" \
  --output_json "$OUT_ROOT/teacher_student_distill_chain_audit.json"
...
"$PYTHON_BIN" "$ROOT/scripts/train_phaseA_alignment_v3_closeability.py" \
  ...
  --lambda_closeability "${STAGEA_LAMBDA_CLOSEABILITY:-0.40}" \
  --lambda_corrective "${STAGEA_LAMBDA_CORRECTIVE:-0.30}" \
  --lambda_score "${STAGEA_LAMBDA_SCORE:-0.10}" \
  --lambda_far_negative_ready "${STAGEA_LAMBDA_FAR_NEG_READY:-0.10}" \
  --auto_closeability_pos_weight \
  --auto_corrective_class_weights \
  --corrective_focus_only \
  --corrective_focus_boost "${CORRECTIVE_FOCUS_BOOST:-2.0}" \
  --corrective_yaw_symmetry_period "${CORRECTIVE_YAW_SYMMETRY_PERIOD:-1.5707963267948966}" \
  --closeability_gate_min "${CLOSEABILITY_GATE_MIN:-0.58}" \
  --closeability_calibrated_gate_min "${CLOSEABILITY_CALIBRATED_GATE_MIN:-0.58}" \
  --use_calibrated_closeability_gate \
...
"$PYTHON_BIN" "$ROOT/scripts/train_phaseA_alignment_v3_closeability.py" \
  ...
  --lambda_closeability "${STAGEB_LAMBDA_CLOSEABILITY:-0.45}" \
  --lambda_corrective "${STAGEB_LAMBDA_CORRECTIVE:-0.35}" \
...
"$PYTHON_BIN" "$ROOT/scripts/summarize_phaseA_alignment_v3_reports.py" \
  --root "$OUT_ROOT" \
  --dataset_report "$OUT_ROOT/alignment_v3_dataset_report.json" \
