...
class AlignmentV2Dataset(Dataset):
...
class PairwiseAlignmentV2Dataset(Dataset):
    def __init__(self, dataset: AlignmentV2Dataset, indices: list[int]):
...
def split_by_episode(dataset: AlignmentV2Dataset, val_ratio: float, seed: int):
...
def cap_far_negative_val_rows(dataset: AlignmentV2Dataset, val_idx: list[int], cap_per_episode: int, seed: int) -> list[int]:
...
def pair_indices_for_split(dataset: AlignmentV2Dataset, indices: list[int]) -> list[int]:
    ...
    torch.save(
        {
            ...
            "alignment_version": "boundary_progress_v2",
...
def main() -> int:
    ...
    dataset = AlignmentV2Dataset(args.dataset_npz)
    ...
    pair_loader = (
        DataLoader(PairwiseAlignmentV2Dataset(dataset, train_pair_idx), batch_size=args.batch_size, shuffle=True, num_workers=0)
        if train_pair_idx
        else None
    )
    val_pair_loader = (
        DataLoader(PairwiseAlignmentV2Dataset(dataset, val_pair_idx), batch_size=args.batch_size, shuffle=False, num_workers=0)
        if val_pair_idx
        else None
    )
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        if best_geometry is None or geometry_score < best_geometry:
            ...
            save_ckpt(output_dir / "student_handoff_state_head_v2_alignment_v2_best_geometry.pt", model, args, val_metrics, history)
        if best_progress is None or progress_score < best_progress:
            best_progress = progress_score
            save_ckpt(output_dir / "student_handoff_state_head_v2_alignment_v2_best_progress.pt", model, args, val_metrics, history)
        if best_deploy is None or deploy_score < best_deploy:
            best_deploy = deploy_score
            save_ckpt(output_dir / "student_handoff_state_head_v2_alignment_v2_best_deploy_candidate.pt", model, args, val_metrics, history)
    ...
    save_ckpt(output_dir / "student_handoff_state_head_v2_alignment_v2_final.pt", model, args, final_metrics, history)
    (output_dir / "alignment_v2_train_history.json").write_text(json.dumps(history, indent=2))

    best_deploy_ckpt = torch.load(output_dir / "student_handoff_state_head_v2_alignment_v2_best_deploy_candidate.pt", map_location="cpu")
    ...
    (output_dir / "alignment_v2_gate_report.json").write_text(json.dumps(gate, indent=2))
