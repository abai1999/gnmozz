...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/runtime_artifacts/depth_force_contact/ablation_C_nearzone_medium_3ep_20260504a}"
...
exec env xvfb-run -a "$PYTHON_BIN" scripts/evaluate_rlbench_modes.py \
  ...
  --name_suffix "depth_force_local_proposal_ablation_C_nearzone_medium_3ep" \
  ...
  --enable_alignment_close_veto --disable_close_latch \
  --enable_alignment_near_zone_gate \
  --alignment_near_zone_xy_threshold 0.05 --alignment_near_zone_z_threshold 0.10 \
  --learned_residual_scale 0.5 --max_residual_pos 0.003 --max_residual_rot 0.020 \
