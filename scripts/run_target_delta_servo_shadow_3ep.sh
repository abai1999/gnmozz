...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/runtime_artifacts/depth_force_contact/target_delta_servo_shadow_3ep_20260506a}"
...
exec env xvfb-run -a "$PYTHON_BIN" scripts/evaluate_rlbench_modes.py \
  ...
  --name_suffix "target_delta_servo_shadow_3ep" \
  ...
  --enable_target_delta_servo_shadow \
  --target_delta_servo_source predictor \
  --target_delta_servo_k_xy 0.08 \
  --target_delta_servo_k_z 0.06 \
  --target_delta_servo_k_yaw 0.04 \
  --target_delta_servo_max_pos 0.0010 \
  --target_delta_servo_max_yaw 0.0040 \
