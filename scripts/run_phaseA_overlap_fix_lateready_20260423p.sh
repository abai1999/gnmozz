...
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT/runtime_artifacts/stage_refiner/student_handoff_state_v2_phaseA_overlapfix_lateready_20260423p/train_overlapfix_lateready_small}"
...
CONSISTENCY_CKPT="${CONSISTENCY_CKPT:-$ROOT/runtime_artifacts/stage_refiner/student_handoff_state_v2_phaseA_overlapfix_zaware_20260423o/train_overlapfix_zaware_small/student_handoff_state_head_v2_best_ready.pt}"
...
python "$ROOT/scripts/train_student_handoff_state_head_v2.py" \
  ...
  --lambda_close_ready_crossing 0.30 \
  --crossing_xy_max 1.05 \
  --crossing_z_max 1.00 \
  --crossing_yaw_max 1.00 \
  --lambda_z_aware_ready_crossing 0.25 \
  --z_ready_xy_near_max 1.10 \
  --z_ready_z_max 0.90 \
  --z_ready_yaw_max 1.00 \
  --lambda_late_ready_logit_lift 0.45 \
  --late_ready_xy_max 1.00 \
  --late_ready_z_max 1.00 \
  --late_ready_yaw_max 1.00 \
  --late_ready_uncertainty_max 0.50 \
  --late_ready_prob_min 0.40 \
  --late_ready_prob_max 0.50 \
...
echo "[phaseA-overlap-fix-lateready] output_dir=$OUTPUT_DIR"
