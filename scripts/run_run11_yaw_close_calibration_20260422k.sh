...
INIT_CKPT="${INIT_CKPT:-$ROOT/runtime_artifacts/stage_refiner/phaseA_runtime_candidates_20260422k/run10_best_phaseA_deploy.pt}"
...
DATA_STAGE1A_DIR="${DATA_STAGE1A_DIR:-$ROOT/runtime_artifacts/stage_refiner/student_handoff_state_dataset_v2_phaseA_run11_stage1a_20260422k}"
DATA_STAGE1B_BASE_DIR="${DATA_STAGE1B_BASE_DIR:-$ROOT/runtime_artifacts/stage_refiner/student_handoff_state_dataset_v2_phaseA_run11_stage1b_base_20260422k}"
DATA_STAGE1B_MERGED_DIR="${DATA_STAGE1B_MERGED_DIR:-$ROOT/runtime_artifacts/stage_refiner/student_handoff_state_dataset_v2_phaseA_run11_stage1b_merged_20260422k}"

RUN11_ROOT="${RUN11_ROOT:-$ROOT/runtime_artifacts/stage_refiner/student_handoff_state_v2_phaseA_20260422k}"
RUN11_STAGE1A_OUT_DIR="${RUN11_STAGE1A_OUT_DIR:-$RUN11_ROOT/train_run11_stage1a_calibwarm}"
RUN11_STAGE1B_OUT_DIR="${RUN11_STAGE1B_OUT_DIR:-$RUN11_ROOT/train_run11_stage1b_calibmix}"

mkdir -p "$DATA_STAGE1A_DIR" "$DATA_STAGE1B_BASE_DIR" "$DATA_STAGE1B_MERGED_DIR" "$RUN11_STAGE1A_OUT_DIR" "$RUN11_STAGE1B_OUT_DIR"

if [[ -f "$DATA_STAGE1A_DIR/handoff_state_dataset_v2_supportmix.npz" ]]; then
  echo "[run11-stage1a] reuse existing calibration-biased support mix"
else
  echo "[run11-stage1a] build calibration-biased support mix"
  "$PYTHON_BIN" "$ROOT/scripts/build_student_handoff_state_dataset_v2.py" \
    ...
    --support_npz "$SUPPORT_TARGETED16" --source_name targeted16 --source_weight_mult 1.00 \
    --support_npz "$SUPPORT_LEARNED32" --source_name learned32 --source_weight_mult 0.55 \
    --support_npz "$SUPPORT_XYRECOVERY" --source_name xyrecovery_parallel7 --source_weight_mult 0.35 \
    --support_npz "$SUPPORT_TRUTHREADY_POS" --source_name truthready_positive --source_weight_mult 1.10 \
...
if [[ -f "$DATA_STAGE1A_DIR/handoff_state_dataset_v2_mixed.npz" ]]; then
  echo "[run11-stage1a] reuse existing merged stage1a dataset"
else
  echo "[run11-stage1a] merge support mix + anchor windows"
  "$PYTHON_BIN" "$ROOT/scripts/merge_student_handoff_state_datasets_v2.py" \
    ...
    --dataset_npz "$ANCHOR_WINDOW_DATASET" --weight_mult 2.40 \
...
env CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" PYTHONUNBUFFERED=1 OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 OPENBLAS_NUM_THREADS=4 NUMEXPR_NUM_THREADS=4 \
  ...
  --output_dir "$RUN11_STAGE1A_OUT_DIR" \
  --epochs 2 \
  --batch_size 64 \
  --lr 1.2e-4 \
  --val_ratio 0.2 \
  ...
  --deploy_false_ready_max 0.005 \
  --init_ckpt "$INIT_CKPT" \
  | tee "$RUN11_STAGE1A_OUT_DIR/stdout.log"

STAGE1A_DEPLOY="$RUN11_STAGE1A_OUT_DIR/student_handoff_state_head_v2_best_phaseA_deploy.pt"
if [[ ! -f "$STAGE1A_DEPLOY" ]]; then
  STAGE1A_DEPLOY="$RUN11_STAGE1A_OUT_DIR/student_handoff_state_head_v2_best_anchor.pt"
fi
if [[ ! -f "$STAGE1A_DEPLOY" ]]; then
  STAGE1A_DEPLOY="$RUN11_STAGE1A_OUT_DIR/student_handoff_state_head_v2_best.pt"
fi

if [[ -f "$DATA_STAGE1B_BASE_DIR/handoff_state_dataset_v2_supportmix.npz" ]]; then
  echo "[run11-stage1b] reuse existing close-neighborhood calibration mix"
else
  echo "[run11-stage1b] build close-neighborhood calibration mix"
  "$PYTHON_BIN" "$ROOT/scripts/build_student_handoff_state_dataset_v2.py" \
    --support_npz "$SUPPORT_LEARNED32" --source_name learned32 --source_weight_mult 0.65 \
    --support_npz "$SUPPORT_TARGETED16" --source_name targeted16 --source_weight_mult 1.35 \
    --support_npz "$SUPPORT_XYRECOVERY" --source_name xyrecovery_parallel7 --source_weight_mult 0.45 \
    --support_npz "$SUPPORT_LATEPROFILE" --source_name lateprofile_parallel8 --source_weight_mult 1.75 \
    --support_npz "$SUPPORT_LATEPROFILE_NEXTPAR" --source_name lateprofile_next_parallel8 --source_weight_mult 0.90 \
    --support_npz "$SUPPORT_LATEPROFILE_READYPOS" --source_name lateprofile_readypos_parallel8 --source_weight_mult 2.00 \
    --support_npz "$SUPPORT_TRUTHREADY_POS" --source_name truthready_positive --source_weight_mult 1.00 \
...
if [[ -f "$DATA_STAGE1B_MERGED_DIR/handoff_state_dataset_v2_mixed.npz" ]]; then
  echo "[run11-stage1b] reuse existing merged stage1b dataset"
else
  echo "[run11-stage1b] merge soft full mix + anchor windows"
  "$PYTHON_BIN" "$ROOT/scripts/merge_student_handoff_state_datasets_v2.py" \
    --dataset_npz "$DATA_STAGE1B_BASE_DIR/handoff_state_dataset_v2_supportmix.npz" --weight_mult 1.0 \
    --dataset_npz "$ANCHOR_WINDOW_DATASET" --weight_mult 1.80 \
...
env CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" PYTHONUNBUFFERED=1 OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 OPENBLAS_NUM_THREADS=4 NUMEXPR_NUM_THREADS=4 \
  ...
  --lambda_consistency_band 0.16 \
  --lambda_consistency_ready 0.14 \
  --init_ckpt "$STAGE1A_DEPLOY" \
  | tee "$RUN11_STAGE1B_OUT_DIR/stdout.log"
