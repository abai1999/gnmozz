...
TAG="${TAG:-20260518a_closebridge}"

PHASE1_BASE_RAW="${PHASE1_BASE_RAW:-$ROOT/runtime_artifacts/alignment_tc_diffusion/privileged_teacher_raw_20260518a_edgepair_label_only_phase1_50ep/alignment_tc_privileged_teacher_raw_20260518a_edgepair_label_only_phase1_50ep.npz}"
...
"$PYTHON_BIN" "$ROOT/scripts/build_alignment_tc_student_vnext_dataset.py" \
  ...
  --close_ready_pre_window_steps 8 \
  --close_ready_post_window_steps 4 \
  --close_ready_bridge_weight 2.75 \
  --close_ready_exact_weight 4.50 \
  --handoff_ready_weight_boost 2.25
