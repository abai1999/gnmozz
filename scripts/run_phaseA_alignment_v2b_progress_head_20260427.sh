...
OUT_ROOT="${OUT_ROOT:-$ROOT/runtime_artifacts/stage_refiner/phaseA_alignment_v2b_progress_head_20260427}"
...
STAGEA_DIR="$TRAIN_DIR/stageA_progress_head"
STAGEB_DIR="$TRAIN_DIR/stageB_counterfactual_progress_head"

mkdir -p "$DATASET_DIR" "$STAGEA_DIR" "$STAGEB_DIR"

echo "[alignment-v2b] OUT_ROOT=$OUT_ROOT"
echo "[alignment-v2b] RECOLLECT_SUPPORT_NPZ=$RECOLLECT_SUPPORT_NPZ"
echo "[alignment-v2b] INIT_CKPT=$INIT_CKPT"
...
"$PYTHON_BIN" "$ROOT/scripts/train_phaseA_alignment_v2_boundary_progress.py" \
  ...
  --lambda_progress "${STAGEA_LAMBDA_PROGRESS:-0.60}" \
...
"$PYTHON_BIN" "$ROOT/scripts/train_phaseA_alignment_v2_boundary_progress.py" \
  ...
  --lambda_progress "${STAGEB_LAMBDA_PROGRESS:-0.50}" \
...
cp "$STAGEB_DIR/alignment_v2_gate_report.json" "$OUT_ROOT/alignment_v2b_gate_report.json"
...
echo "[alignment-v2b] complete"
echo "[alignment-v2b] gate_report=$OUT_ROOT/alignment_v2b_gate_report.json"
echo "[alignment-v2b] deploy_candidate=$STAGEB_DIR/student_handoff_state_head_v2_alignment_v2_best_deploy_candidate.pt"
