...
ALLOWED_SELF_PREFIXES = (
    "_b2_candidate_shadow",
    "_last_b2_candidate_shadow",
)
...
FORBIDDEN_SHADOW_SELF_CALLS = {
    "_apply_alignment_close_veto",
    "_close_veto_ready",
    "_close_veto_geometry_ready",
}
...
def _is_shadow_call(node: ast.AST) -> bool:
    return (
        ...
        and node.func.attr == "_run_b2_candidate_evaluator_shadow"
    )
...
def main() -> int:
    ...
    consumed_call_nodes: list[ast.AST] = []

    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and node.name == "_run_b2_candidate_evaluator_shadow":
            shadow_fn = node
        if _is_shadow_call(node):
            call_nodes.append(node)

    if shadow_fn is None:
        errors.append("_run_b2_candidate_evaluator_shadow is missing")
    else:
        for node in ast.walk(shadow_fn):
            ...
            if (
                ...
                and node.func.attr in FORBIDDEN_SHADOW_SELF_CALLS
            ):
                errors.append(f"shadow calls side-effecting self.{node.func.attr} at line {node.lineno}")

    for parent in ast.walk(tree):
        for child in ast.iter_child_nodes(parent):
            if _is_shadow_call(child) and not isinstance(parent, ast.Expr):
                consumed_call_nodes.append(child)

    if not call_nodes:
        errors.append("_run_b2_candidate_evaluator_shadow is never called")
    if consumed_call_nodes:
        for node in consumed_call_nodes:
