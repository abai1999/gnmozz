...
OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/runtime_artifacts/depth_force_contact/alignment_v3_from_scratch_apply_3ep_20260509a}"
...
ALIGNMENT_V3_CKPT="${ALIGNMENT_V3_CKPT:-$ROOT/runtime_artifacts/depth_force_contact/alignment_v3_from_scratch_formal_run1_20260508a/alignment_v3_direct_local_best.pt}"
...
ALIGNMENT_V3_APPLY_SCALE="${ALIGNMENT_V3_APPLY_SCALE:-1.0}"
ALIGNMENT_V3_APPLY_MAX_POS="${ALIGNMENT_V3_APPLY_MAX_POS:-0.0020}"
ALIGNMENT_V3_APPLY_MAX_YAW="${ALIGNMENT_V3_APPLY_MAX_YAW:-0.0020}"
ALIGNMENT_V3_APPLY_REQUIRE_IMPROVE="${ALIGNMENT_V3_APPLY_REQUIRE_IMPROVE:-0}"

mkdir -p "$OUTPUT_ROOT"

EXTRA_ARGS=()
if [[ "$ALIGNMENT_V3_APPLY_REQUIRE_IMPROVE" == "1" || "$ALIGNMENT_V3_APPLY_REQUIRE_IMPROVE" == "true" ]]; then
  EXTRA_ARGS+=(--alignment_v3_apply_require_improve)
fi
...
exec env xvfb-run -a "$PYTHON_BIN" scripts/evaluate_rlbench_modes.py \
  ...
  --name_suffix "alignment_v3_from_scratch_apply_3ep" \
  --alignment_ckpt "$ALIGNMENT_CKPT" \
  --alignment_v3_shadow_ckpt "$ALIGNMENT_V3_CKPT" \
  --enable_alignment_v3_apply \
  --alignment_v3_apply_scale "$ALIGNMENT_V3_APPLY_SCALE" \
  --alignment_v3_apply_max_pos "$ALIGNMENT_V3_APPLY_MAX_POS" \
  --alignment_v3_apply_max_yaw "$ALIGNMENT_V3_APPLY_MAX_YAW" \
