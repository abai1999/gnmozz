...
OUT_ROOT="${OUT_ROOT:-$ROOT/runtime_artifacts/stage_refiner/phaseA_runtime_ready_v20260426a/recollection}"
RUN_NAME_SUFFIX="${RUN_NAME_SUFFIX:-phaseA_runtime_ready_recollect_v20260426a}"
...
RECOLLECTION_EPISODES="${RECOLLECTION_EPISODES:-$("$PYTHON_BIN" - <<'PY'
import random
seed = 3407
base = [44,46,11,6,29,33,21,14,35,36,12,43,18,34,45,13]
remaining = [x for x in range(50) if x not in base]
rng = random.Random(seed)
extra = sorted(rng.sample(remaining, 4))
eps = base + extra
...
IFS=',' read -r -a EP_ARR <<< "$RECOLLECTION_EPISODES"
...
for ((g=0; g<N_GPU; g++)); do
  ...
  echo "[phaseA-runtime-ready-recollect] launch $shard_name episodes=$eps"
...
echo "[phaseA-runtime-ready-recollect] launched pids: ${PIDS[*]}"
for pid in "${PIDS[@]}"; do
  wait "$pid"
done
...
{
  "recollection_episode_indices_csv": "$RECOLLECTION_EPISODES",
  "support_states_npz": $(printf '%s\n' "${SUPPORT_NPZS[@]}" | "$PYTHON_BIN" - <<'PY'
import json, sys
print(json.dumps([line.strip() for line in sys.stdin if line.strip()]))
PY
),
  "merged_trace_dir": "$MERGED_OUT_DIR"
}
JSON

echo "[phaseA-runtime-ready-recollect] merged traces at $MERGED_OUT_DIR"
echo "[phaseA-runtime-ready-recollect] support npzs: ${SUPPORT_NPZS[*]}"
