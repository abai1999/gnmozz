...
OUT_ROOT="${OUT_ROOT:-$ROOT/eval_logs/insert_onto_square_peg/b1b2_candidate_recollect_20260423y}"
SUPPORT_OUT_DIR="${SUPPORT_OUT_DIR:-$ROOT/runtime_artifacts/residual_data/insert_phase1_b1b2_candidate_recollect_20260423y}"
RUN_NAME_SUFFIX="${RUN_NAME_SUFFIX:-b1b2_candidate_recollect_20260423y}"

# Current-profile candidate recollection targets:
# - xy-edge / xy-block: 16,22,23,46,14
# - yaw-needed / yaw-boundary: 14,18,23,26,34,36,45,46
# - teacher-ready anchors: 6,8,11,23,28,35
EPISODE_INDICES="${EPISODE_INDICES:-16,22,23,46,14,18,26,34,36,45,6,8,11,28,35}"
...
# This collection is explicitly not deployment runtime.  It is a teacher-assisted
# current-profile candidate-label collection: privileged information is only for
# steering/labels, and downstream builders must window-filter it before training.
...
echo "[b1b2-candidate-recollect] launched pid=$(cat "$SUPPORT_OUT_DIR/logs/driver.pid")"
echo "[b1b2-candidate-recollect] log=$LOG"
echo "[b1b2-candidate-recollect] episodes=$EPISODE_INDICES gpus=$GPU_IDS"
