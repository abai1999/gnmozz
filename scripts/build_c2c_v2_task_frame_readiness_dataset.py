...
def build_readiness_row(
    ...
    *,
    ...
    z_ready = bool(z_observable and z_near and z_contact_or_depth_ready)
    yaw_ready = bool(yaw_observable and not yaw_ambiguous and yaw_near)
    handoff_ready = bool(xy_ready and z_ready and yaw_ready and z_observable and yaw_observable)
    return {
        ...
        "runtime_features": {
            ...
            "runtime_xy_confidence": _safe_float(runtime_xy.get("confidence"), 0.0),
            "runtime_xy_entry_ready": bool(runtime_xy.get("entry_ready", False)),
            "runtime_xy_step_scale": _safe_float(runtime_xy.get("xy_step_scale", 1.0), 1.0),
        ...
        "offline_labels": {
            ...
            "alignment_xy_ready": bool(xy_ready),
            "z_observable": bool(z_observable),
            "z_observable_label": bool(z_observable),
            "z_near_alignment": bool(z_near),
            "z_near_alignment_label": bool(z_near),
            "z_contact_or_depth_ready": bool(z_contact_or_depth_ready),
            "z_contact_or_depth_ready_label": bool(z_contact_or_depth_ready),
            "z_ready": bool(z_ready),
            "z_ready_label": bool(z_ready),
            "z_ready_semantics": "task_frame_z_readiness",
            "z_abstain_reason": "" if z_observable else "z_not_observable",
            "yaw_observable": bool(yaw_observable),
            "yaw_observable_label": bool(yaw_observable),
            "yaw_ambiguous": bool(yaw_ambiguous),
            "yaw_ambiguous_label": bool(yaw_ambiguous),
            "yaw_unobservable": bool(yaw_unobservable),
            "yaw_unobservable_label": bool(yaw_unobservable),
            "yaw_near_alignment": bool(yaw_near),
            "yaw_ready": bool(yaw_ready),
            "yaw_ready_label": bool(yaw_ready),
            "yaw_ready_semantics": "task_frame_yaw_readiness",
...
def summarize(rows: list[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "z_ready_rate": float(np.mean([bool(l["z_ready"]) for l in labels])) if labels else 0.0,
        "yaw_observable_rate": float(np.mean([bool(l["yaw_observable"]) for l in labels])) if labels else 0.0,
        "yaw_ambiguous_rate": float(np.mean([bool(l["yaw_ambiguous"]) for l in labels])) if labels else 0.0,
        "yaw_near_alignment_rate": float(np.mean([bool(l["yaw_near_alignment"]) for l in labels])) if labels else 0.0,
        "yaw_ready_rate": float(np.mean([bool(l["yaw_ready"]) for l in labels])) if labels else 0.0,
