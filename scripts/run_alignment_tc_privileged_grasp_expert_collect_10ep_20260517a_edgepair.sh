...
TAG="${TAG:-20260517a_edgepair_grasp_10ep}"
...
env xvfb-run -a "$PYTHON_BIN" "$ROOT/scripts/evaluate_rlbench.py" \
  ...
  --teacher_close_xy_threshold 0.006 \
  --teacher_close_abs_z_threshold 0.010 \
  --teacher_close_yaw_threshold 0.05 \
  --teacher_close_contact_depth_threshold 0.022 \
  --teacher_grasp_ready_threshold 0.50 \
  --teacher_grasp_xy_threshold 0.006 \
  --teacher_grasp_abs_z_threshold 0.010 \
  --teacher_grasp_yaw_threshold 0.05 \
  --teacher_verify_hold_steps 10 \
  --teacher_verify_settle_steps 4 \
