#!/usr/bin/env python3
"""Build a targeted large-bias augmentation dataset for Coarse2Contact v2 recovery training."""
...
from prismatic.robot.coarse2contact_v2.recovery_augmentation import build_targeted_augmented_records, load_bias_template
...
def main() -> None:
    ...
    ap.add_argument("--output_root", type=Path, default=Path("runtime_artifacts/coarse2contact_v2/datasets_recovery_targeted_aug"))
    ap.add_argument("--hard_fraction", type=float, default=0.35)
    ap.add_argument("--min_trajectories", type=int, default=6)
    ap.add_argument("--scales", type=str, default="1.25,1.5,1.75")
    args = ap.parse_args()
    ...
    combined, report = build_targeted_augmented_records(
        base_records,
    ...
    out_path = out_root / "grasp_recovery_dataset_v2_targeted_aug.jsonl"
    ...
    (out_root / "grasp_recovery_dataset_v2_targeted_aug_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    print(out_path)
    print(out_root / "grasp_recovery_dataset_v2_targeted_aug_summary.json")
