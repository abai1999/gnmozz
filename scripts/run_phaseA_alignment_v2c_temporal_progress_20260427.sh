...
OUT_ROOT="${OUT_ROOT:-$ROOT/runtime_artifacts/stage_refiner/phaseA_alignment_v2c_temporal_progress_20260427}"
...
STAGEA_DIR="$TRAIN_DIR/stageA_temporal_progress"
STAGEB_DIR="$TRAIN_DIR/stageB_temporal_counterfactual"

mkdir -p "$DATASET_DIR" "$STAGEA_DIR" "$STAGEB_DIR"

echo "[alignment-v2c] OUT_ROOT=$OUT_ROOT"
echo "[alignment-v2c] RECOLLECT_SUPPORT_NPZ=$RECOLLECT_SUPPORT_NPZ"
echo "[alignment-v2c] RECOLLECT_SUPPORT_NPZ_EXTRA=${RECOLLECT_SUPPORT_NPZ_EXTRA:-}"
echo "[alignment-v2c] INIT_CKPT=$INIT_CKPT"
...
"$PYTHON_BIN" "$ROOT/scripts/audit_phaseA_alignment_v2_progress.py" \
...
"$PYTHON_BIN" "$ROOT/scripts/train_phaseA_alignment_v2_boundary_progress.py" \
  ...
  --lambda_progress "${STAGEA_LAMBDA_PROGRESS:-0.70}" \
  ...
  --progress_gate_min "${PROGRESS_GATE_MIN:-0.58}" \
  --progress_neg_recall_gate_min "${PROGRESS_NEG_RECALL_GATE_MIN:-0.45}"
...
"$PYTHON_BIN" "$ROOT/scripts/train_phaseA_alignment_v2_boundary_progress.py" \
  ...
  --lambda_progress "${STAGEB_LAMBDA_PROGRESS:-0.60}" \
...
cp "$STAGEB_DIR/alignment_v2_gate_report.json" "$OUT_ROOT/alignment_v2c_gate_report.json"

"$PYTHON_BIN" "$ROOT/scripts/audit_phaseA_alignment_v2_progress.py" \
  --dataset_npz "$FULL_NPZ" \
  --ckpt "$STAGEB_DIR/student_handoff_state_head_v2_alignment_v2_best_deploy_candidate.pt" \
  --output_dir "$OUT_ROOT"
...
echo "[alignment-v2c] complete"
echo "[alignment-v2c] gate_report=$OUT_ROOT/alignment_v2c_gate_report.json"
echo "[alignment-v2c] deploy_candidate=$STAGEB_DIR/student_handoff_state_head_v2_alignment_v2_best_deploy_candidate.pt"
