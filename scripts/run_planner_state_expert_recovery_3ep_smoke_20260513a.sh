...
OUT_DIR="${OUT_DIR:-$ROOT/runtime_artifacts/alignment_tc_diffusion/planner_state_expert_recovery_20260513a_3ep_smoke}"
...
"${RUN_PREFIX[@]}" "$PYTHON" "$ROOT/scripts/collect_planner_state_expert_recovery.py" \
  ...
  --record_video \
  --output_dir "$OUT_DIR" \
  --output_npz "$OUT_DIR/alignment_tc_planner_state_expert_recovery_raw_20260513a_3ep.npz" \
  --report_json "$OUT_DIR/report_20260513a_3ep.json" \
  --takeover_trace_jsonl "$OUT_DIR/takeover_trace_20260513a_3ep.jsonl"
