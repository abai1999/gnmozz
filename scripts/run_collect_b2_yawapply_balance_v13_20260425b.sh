...
SCRATCH_ROOT="${SCRATCH_ROOT:-/mnt/ssd/guoning/VLA_scratch/b2_yawapply_balance_v13_20260425b}"
OUT_ROOT="${OUT_ROOT:-$SCRATCH_ROOT/eval}"
SUPPORT_OUT_DIR="${SUPPORT_OUT_DIR:-$SCRATCH_ROOT/support}"
RUN_NAME_SUFFIX="${RUN_NAME_SUFFIX:-b2_yawapply_balance_v13_20260425b}"

# Focus on the remaining B2 yaw blockers:
# - ep018: currently keep=runtime_like, apply=targeted_yawapply; needs same-profile keep/apply.
# - ep23/45: oracle-skew or weak current-profile apply in previous audits.
# - ep22/34/36: likely additional apply candidates.
# - ep12/14/46: preserve existing apply diversity and same-profile anchors.
EPISODE_INDICES="${EPISODE_INDICES:-18,23,45,22,34,36,12,14,46}"
...
# This is a teacher-assisted label/recovery collection, not deployment runtime.
# It is deliberately milder than the earlier yawapply top-up so the same
# targeted_yawapply profile can contain both keep and apply windows.
...
--teacher_smooth_kp_xy 0.58 \
--teacher_smooth_kp_z 0.56 \
--teacher_smooth_kp_yaw 0.10 \
--teacher_smooth_yaw_deadband 0.024 \
--teacher_max_yaw_step 0.0035 \
...
echo "[b2-yawapply-balance-v13] launched pid=$(cat "$SUPPORT_OUT_DIR/logs/driver.pid")"
echo "[b2-yawapply-balance-v13] log=$LOG"
echo "[b2-yawapply-balance-v13] episodes=$EPISODE_INDICES gpus=$GPU_IDS"
