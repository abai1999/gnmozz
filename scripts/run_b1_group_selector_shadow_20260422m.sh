...
B1_SELECTOR_CKPT="${B1_SELECTOR_CKPT:-$ROOT/runtime_artifacts/stage_refiner/student_group_selector_v2_b1_run12_20260422m/student_group_selector_v2_best.pt}"

OUTPUT_ROOT="${OUTPUT_ROOT:-$ROOT/eval_logs/insert_onto_square_peg/b1_group_selector_shadow_20260422m}"
NAME_SUFFIX="${NAME_SUFFIX:-run12_b1_shadow}"
EPISODE_INDICES="${EPISODE_INDICES:-11,6,9,19,14,17,12,1}"
...
exec env xvfb-run -a "$PYTHON_BIN" scripts/evaluate_rlbench_modes.py \
  ...
  --student_group_selector_shadow_ckpt "$B1_SELECTOR_CKPT" \
  --student_group_selector_handoff_ckpt "$HANDOFF_PROVIDER_CKPT" \
  --planner_no_depth \
  --planner_no_force \
