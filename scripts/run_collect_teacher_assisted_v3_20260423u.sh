...
OUT_ROOT="${OUT_ROOT:-$ROOT/eval_logs/insert_onto_square_peg/current_profile_teacher_assisted_v3_20260423u}"
SUPPORT_OUT_DIR="${SUPPORT_OUT_DIR:-$ROOT/runtime_artifacts/residual_data/insert_phase1_current_profile_teacher_assisted_v3_20260423u}"
RUN_NAME_SUFFIX="${RUN_NAME_SUFFIX:-current_profile_teacher_assisted_v3_20260423u}"

# Focus episodes: very-near/xy-block core + ready anchors.
EPISODE_INDICES="${EPISODE_INDICES:-16,26,25,14,2,34,11,29}"
GPU_IDS="${GPU_IDS:-2,3,4}"
...
--teacher_smooth_kp_xy 0.55 \
--teacher_smooth_kp_z 0.60 \
--teacher_smooth_kp_yaw 0.22 \
--teacher_smooth_yaw_deadband 0.020 \
--teacher_max_yaw_step 0.010 \
...
echo "[teacher-assisted-v3] launched pid=$(cat "$SUPPORT_OUT_DIR/logs/driver.pid")"
echo "[teacher-assisted-v3] log=$LOG"
echo "[teacher-assisted-v3] episodes=$EPISODE_INDICES gpus=$GPU_IDS"
